...
function _exec_in_docker {
  ...
  TOOLBOX_DOCKER_ENTRYPOINT=${TOOLBOX_DOCKER_ENTRYPOINT:-sh}
  ...
  if [ "${TOOLBOX_DOCKER_SSH_FORWARD}" == "true" ]; then
    case "$OSTYPE" in
      ...
      *)  DOCKER_SSH_AUTH_SOCK_FORWARD_PARAMS=${DOCKER_SSH_AUTH_SOCK_FORWARD_PARAMS:---volumes-from=ssh-agent -e SSH_AUTH_SOCK=/.ssh-agent/socket} ;;
    esac
  ...
  local run_cmd=("${TOOLBOX_DOCKER_RUN}" \
    ...
    --entrypoint=${TOOLBOX_DOCKER_ENTRYPOINT} \
